...
public class PessoaResource implements
    ...
    @Override
    public Pessoa get(@RequestBody Pessoa entity) throws Exception {
        return this.pessoaService.read(entity);
    }

    @Override
    @PutMapping(value = "/{id}",
            consumes = {MediaType.APPLICATION_JSON_VALUE})
    public Pessoa put(@RequestBody @Validated Pessoa entity,
                    @PathVariable("id") Integer id) {
        LOGGER.log(Level.INFO,
                String.format("Exemplo do PUT: %s | %d", entity, id));

        return this.pessoaService.updatePut(entity, id);
    ...
    @PatchMapping(value = "/{id}",
            consumes = {MediaType.APPLICATION_JSON_VALUE})
    public Pessoa patch(@RequestBody @Validated Pessoa entity,
                      @PathVariable("id") Integer id) throws Exception {
        LOGGER.log(Level.INFO,
                String.format("Exemplo do PATCH: %s | %d", entity, id));

        return this.pessoaService.updatePatch(entity, id);
